...
func BenchmarkSeqRangeMapFilterLarge(b *testing.B) {
	for n := 0; n < b.N; n++ {
		count := 0
		values := FilterSeq(
			MapSeq(RangeSeq(0, 1_000_000), func(v int) int {
				return v + 1
			}),
			func(v int) bool {
				return v%2 == 0
...
func BenchmarkSeqProduct26Repeat4Large(b *testing.B) {
	for n := 0; n < b.N; n++ {
		count := 0
		for range ProductSeq(AsciiLowercase, 4) {
			count++
		}
...
func BenchmarkSeqProductMapFirstTrueMD5Large(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		joined := MapSeq(ProductSeq(AsciiLowercase, 4), func(value []string) string {
			return strings.Join(value, "")
		})
		result, ok := FirstTrueSeq(joined, func(rawPassword string) bool {
...
func BenchmarkSeqCombinations26Choose5Large(b *testing.B) {
	for n := 0; n < b.N; n++ {
		count := 0
		for range CombinationsSeq(AsciiLowercase, 5) {
			count++
		}
...
func BenchmarkSeqCombinationsWithReplacement26Choose5Large(b *testing.B) {
	for n := 0; n < b.N; n++ {
		count := 0
		for range CombinationsWithReplacementSeq(AsciiLowercase, 5) {
			count++
		}
...
func BenchmarkSeqPermutations10Pick5Large(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		count := 0
		for range PermutationsSeq(symbols, 5) {
			count++
		}
